...
export default {
	...
	methods: {
		...
		changerSalle(){
			...
			axios.put(this.$store.state.url+`/depense/${this.produit.id}/`, this.new_produit, headers)
			.then((response) => {
				let depenses = this.$store.state.depenses;
				for(var i = 0; i < depenses.length; i++){
					let x = depenses[i];
					if(x.id==this.produit.id){
						this.$store.state.depenses[i] = response.data;
						break;
					}
				}
				this.$emit("close");
