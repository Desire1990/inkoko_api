	...
	<div :class="{popup:true, active:visible}" @click="close">
		<div class="popup-body" @click.prevent.stop>
			...
			<form method="post">
				...
				<div class="field">
					<label for="id_unite">Type de poule:</label>
					<input type="text" v-model="new_produit.type_poulle" id="id_unite">
				</div>
				<div class="field">
					<label for="id_prix">Responsable:</label>
					<input type="number" name="prix" required="" id="id_quantite" v-model="new_produit.responsable">
				</div>
				<div class="buttons">
...
export default {
	...
	methods: {
		...
		changerSalle(){
			...
			axios.put(this.$store.state.url+`/salle/${this.produit.id}/`, this.new_produit, headers)
			.then((response) => {
				let salles = this.$store.state.salles;
				for(var i = 0; i < salles.length; i++){
					let x = salles[i];
					if(x.id==this.produit.id){
						this.$store.state.salles[i] = response.data;
						break;
					}
				}
				this.$emit("close");
