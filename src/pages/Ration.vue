...
import Item from "../components/produit_item.vue";
import CartItem from "../components/panier_produit_item.vue";
...
export default{
	...
	computed:{
		non_empty_items(){
			return this.items.filter(x => x.quantite > 0)
		},
		empty_items(){
			return this.items.filter(x => x.quantite == 0)
		}
	...
	mounted(){
		...
		document.onkeydown = (e) => {
			...
			let action = setTimeout(()=>{
				for(let item of this.$store.state.produits){
