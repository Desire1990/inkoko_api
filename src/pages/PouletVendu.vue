        ...
        <table class="table commandes">
          ...
          <tbody id="commandes">
            <tr v-for="commande, count in items">
              ...
              <td>{{(commande.salle)}}</td>              
              <td>{{(commande.client)}}</td>              
              <td>{{money(commande.prix_unitaire)}} Fbu</td>              
              <td>{{(commande.poids)}} kg</td>              
              <td>{{(commande.quantite)}}</td>              
              <td>{{money(commande.prix_unitaire*commande.poids)}} Fbu</td>              
            </tr>
          </tbody>
          <tfoot>
            <tr class="panier-item">
              <th colspan="5">Total</th>
              <th>{{(totale())}} poulets</th>
...
export default {
  ...
  data(){
    return{
      query_date : false,
      items : this.$store.state.pouletVendus,
  ...
  methods:{
    ...
    performFilter(value){
      let data = this.$store.state.pouletVendus.filter( x => {
        return x.id == value |
    ...
    fetchData(){
      ...
      axios.get(this.$store.state.url+'/poulle_vendu/', headers)
      .then((response) => {
        this.$store.state.pouletVendus = response.data.results;
    ...
    filterByDate(e){
      ...
      axios.get(this.$store.state.url+`/poulle_vendu/?du=${since}&au=${to}`, headers)
      .then((response) => {
