...
import Item from "../components/vente_item.vue";
import CartItem from "../components/panier_item.vue";
...
export default{
	...
	mounted(){
		if(this.$store.state.oeufs.length<1){
			this.fetchProducts();
		}
		document.onkeydown = (e) => {
			...
			let action = setTimeout(()=>{
				for(let item of this.$store.state.oeufs){
