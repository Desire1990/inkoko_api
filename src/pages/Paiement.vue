...
import EditDialog from "../components/dialog_edit_paiement.vue";
import ProduitDialog from "../components/dialog_paiement.vue";
...
export default{
	...
	methods:{
      ...
      if(this.$store.state.users.length==0){
        axios.get(this.$store.state.url+'/paiement/', headers)
        .then((response) => {
          this.$store.state.paiements = response.data.results;
		...
		performFilter(value){
			let data = this.$store.state.paiements.filter( x => {
				return x.id == value |
